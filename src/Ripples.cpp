...
public:
  ...
  void Update(const orxCLOCK_INFO &_rstInfo)
  {
    ...
    Ripples &roGame = Ripples::GetInstance();
...
ScrollObject *Ripples::GetInteractionObject() const
{
...
void Ripples::Validate()
{
  orxTEXTURE *pstTexture;

  // Gets validate texture
  pstTexture = orxTexture_Load("ValidateTexture", orxFALSE);
...
void Ripples::DeleteRunTimeObject(const orxSTRING _zObjectName)
{
...
ScrollObject *Ripples::GetRunTimeObject(const orxSTRING _zObjectName) const
{
  ScrollObject *poResult;
...
orxSTATUS Ripples::Bootstrap() const
{
...
void Ripples::UpdateInteraction(const orxCLOCK_INFO &_rstInfo)
{
  ...
  for(orxU32 i = 0, u32Number = orxConfig_GetListCount("PickGroupList"); i < u32Number; i++)
  {
    // Picks object in it
...
void Ripples::UpdateShader(const orxCLOCK_INFO &_rstInfo)
{
  // For all shaders
    ...
    || !orxString_Compare(orxShader_GetName(pstShader), "ShadowViewport"))
    {
      ...
      if(poLevel != orxNULL)
      {
            ...
            poSource = GetNextObject<Source>(poSource))
        {
          ...
          // Gets its texture
          apoTextureList[u32ID] = orxTexture_Load(orxConfig_GetString("Texture"), orxTRUE);
...
void Ripples::UpdateGame(const orxCLOCK_INFO &_rstInfo)
{
  ...
  if(orxInput_IsActive("Success") && orxInput_HasNewStatus("Success"))
  {
    ...
    // Wraps
    if(u32Level >= (orxU32)orxConfig_GetListCount("LevelList"))
    {
      u32Level = 0;
    }
...
void Ripples::UpdatePause(const orxCLOCK_INFO &_rstInfo)
{
  // Pausing?
...
void Ripples::UpdateInput(const orxCLOCK_INFO &_rstInfo)
{
  // Updates pause
  UpdatePause(_rstInfo);
}

void Ripples::Update(const orxCLOCK_INFO &_rstInfo)
{
  // Updates interaction
...
orxSTATUS Ripples::Init()
{
  ...
  // For all viewports
  for(orxU32 i = 0, iCount = orxConfig_GetListCount("ViewportList");
      i < iCount;
      i++)
  {
    // Creates it
...
void Ripples::BindObjects()
{
  // Binds objects
  ScrollBindObject<Source>("O-Source");
}

orxSTATUS orxFASTCALL Ripples::EventHandler(const orxEVENT *_pstEvent)
{
  orxSTATUS eResult = orxSTATUS_SUCCESS;
...
{
  // Executes game
  Ripples::GetInstance().Execute(argc, argv);
